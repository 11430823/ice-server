...
class bench_conf_t
{
	PRIVATE_R_DEFAULT(uint32_t, max_fd_num);//打开fd的最大数量
	PRIVATE_R_DEFAULT(bool, is_daemon);//是否后台运行
	PRIVATE_R_DEFAULT(std::string, liblogic_path);////代码段SO路径
	PRIVATE_R_DEFAULT(uint32_t, log_level);//日志等级
	PRIVATE_R_DEFAULT(time_t, fd_time_out);//连接的FD超时秒数,0:无超时
	PRIVATE_R_DEFAULT(uint32_t, page_size_max);//数据包的最大字节数
	PRIVATE_R_DEFAULT(std::string, log_dir);//日志目录
	PRIVATE_R_DEFAULT(uint32_t, log_save_next_file_interval_min);//每多少时间(分钟)重新保存文件中(新文件)
	PRIVATE_R_DEFAULT(uint32_t, core_size);//core文件的大小，字节
	PRIVATE_R_DEFAULT(uint32_t, restart_cnt_max);//最大重启次数
