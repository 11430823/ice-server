/********************************************************************
	...
	日志文件可以自动轮转。
	注意：如果使用自动轮转,必须保证每天写的日志文件个数不能超过log_init时设定的最大文件个数，否则日志会写乱掉。
...
private:
	lib_log(const lib_log& cr);
	lib_log& operator=(const lib_log& cr);
...
public:
	static void write_log(int lvl,uint32_t key, const char* fmt, ...) LOG_CHECK_FMT(3, 4);
	static void write_syslog(int lvl, const char* fmt, ...) LOG_CHECK_FMT(2, 3);
	static void boot_log(int ok, int dummy, const char* fmt, ...) LOG_CHECK_FMT(3, 4);
};

#ifndef LOG_USE_SYSLOG
#define DETAIL(level, key, fmt, args...) \
		lib_log::write_log(level, key, "[%s][%d]%s: " fmt "\n", __FILE__, __LINE__, __FUNCTION__, ##args)
#else
#define DETAIL(level, key, fmt, args...) \
		ice::lib_log::write_syslog(level, "[%s][%d]%s: " fmt "\n", __FILE__, __LINE__, __FUNCTION__, ##args)
#endif

#ifndef LOG_USE_SYSLOG
#define SIMPLY(level, key, fmt, args...) \
		lib_log::write_log(level, key, fmt "\n", ##args)
#else
#define SIMPLY(level, key, fmt, args...) \
		ice::lib_log::write_syslog(level, fmt "\n", ##args)
#endif
...
#define EMERG_LOG(fmt, args...) \
	DETAIL(lib_log::log_lvl_emerg, 0, fmt, ##args)
/**
 * @def KEMERG_LOG
...
#define KEMERG_LOG(key, fmt, args...) \
		DETAIL(lib_log::log_lvl_emerg, key, fmt, ##args)
#else
#define EMERG_LOG(fmt, args...)
#define KEMERG_LOG(key, fmt, args...) 
#endif
...
#define ALERT_LOG(fmt, args...) \
		DETAIL(lib_log::log_lvl_alert, 0, fmt, ##args)
/**
 * @def KALERT_LOG
...
#define KALERT_LOG(key, fmt, args...) \
		DETAIL(lib_log::log_lvl_alert, key, fmt, ##args)
#else
#define ALERT_LOG(fmt, args...)
#define KALERT_LOG(key, fmt, args...) 
#endif
...
#define CRIT_LOG(fmt, args...) \
		DETAIL(lib_log::log_lvl_crit, 0, fmt, ##args)
/**
 * @def KCRIT_LOG
...
#define KCRIT_LOG(key, fmt, args...) \
		DETAIL(lib_log::log_lvl_crit, key, fmt, ##args)
#else
#define CRIT_LOG(fmt, args...)
#define KCRIT_LOG(key, fmt, args...) 
#endif
...
#define ERROR_LOG(fmt, args...) \
		DETAIL(lib_log::log_lvl_error, 0, fmt, ##args)
/**
 * @def KERROR_LOG
...
#define KERROR_LOG(key, fmt, args...) \
	DETAIL(lib_log::log_lvl_error, key, fmt, ##args)
#else
#define ERROR_LOG(fmt, args...)
#define KERROR_LOG(key, fmt, args...) 
#endif
...
#define WARN_LOG(fmt, args...) \
		SIMPLY(lib_log::log_lvl_warning, 0, fmt, ##args)
/**
 * @def KWARN_LOG
...
#define KWARN_LOG(key, fmt, args...) \
		SIMPLY(lib_log::log_lvl_warning, key, fmt, ##args)
#else
#define WARN_LOG(fmt, args...)
#define KWARN_LOG(key, fmt, args...) 
#endif
...
#define NOTI_LOG(fmt, args...) \
		SIMPLY(lib_log::log_lvl_notice, 0, fmt, ##args)
/**
 * @def KNOTI_LOG
...
#define KNOTI_LOG(key, fmt, args...) \
		SIMPLY(lib_log::log_lvl_notice, key, fmt, ##args)
#else
#define NOTI_LOG(fmt, args...)
#define KNOTI_LOG(key, fmt, args...) 
#endif
...
#define INFO_LOG(fmt, args...) \
		SIMPLY(lib_log::log_lvl_info, 0, fmt, ##args)
/**
 * @def KINFO_LOG
...
#define KINFO_LOG(key, fmt, args...) \
		SIMPLY(lib_log::log_lvl_info, key, fmt, ##args)
#else
#define INFO_LOG(fmt, args...)
#define KINFO_LOG(key, fmt, args...) 
#endif
...
#define DEBUG_LOG(fmt, args...) \
		SIMPLY(lib_log::log_lvl_debug, 0, fmt, ##args)
/**
 * @def KDEBUG_LOG
...
#define KDEBUG_LOG(key, fmt, args...) \
		SIMPLY(lib_log::log_lvl_debug, key, fmt, ##args)
#else
#define DEBUG_LOG(fmt, args...)
#define KDEBUG_LOG(key, fmt, args...) 
#endif
...
#define TRACE_LOG(fmt, args...) \
		DETAIL(lib_log::log_lvl_trace, 0, fmt, ##args)
/**
 * @def KTRACE_LOG
...
#define KTRACE_LOG(key, fmt, args...) \
		DETAIL(lib_log::log_lvl_trace, key, fmt, ##args)
#else
#define TRACE_LOG(fmt, args...)
#define KTRACE_LOG(key, fmt, args...) 
#endif
...
#define BOOT_LOG(OK, fmt, args...) \
		do { \
			lib_log::boot_log(OK, 0, fmt, ##args); \
			return OK; \
		} while (0)
...
#define BOOT_LOG2(OK, n, fmt, args...) \
		do { \
			lib_log::boot_log(OK, n, fmt, ##args); \
			return OK; \
		} while (0)
