...
namespace ice{
	...
	* @return 成功返回0，失败返回-1。
	*/
	int setup_log_by_time(const char* dir, E_LOG_LEVEL lvl, const char* pre_name, uint32_t logtime);
	/**
	...
	* @see log_init
	*/
	void log_set_dest(E_LOG_DEST dest);
...
#define DETAIL(level, key, fmt, args...) \
		ice::write_log(level, key, "[%s][%d]%s: " fmt "\n", __FILE__, __LINE__, __FUNCTION__, ##args)
#else
#define DETAIL(level, key, fmt, args...) \
		ice::write_syslog(level, "[%s][%d]%s: " fmt "\n", __FILE__, __LINE__, __FUNCTION__, ##args)
#endif

#ifndef LOG_USE_SYSLOG
#define SIMPLY(level, key, fmt, args...) \
		ice::write_log(level, key, fmt "\n", ##args)
#else
#define SIMPLY(level, key, fmt, args...) \
		ice::write_syslog(level, fmt "\n", ##args)
#endif
...
#define EMERG_LOG(fmt, args...) \
		DETAIL(ice::log_lvl_emerg, 0, fmt, ##args)
/**
 * @def KEMERG_LOG
...
#define KEMERG_LOG(key, fmt, args...) \
		DETAIL(ice::log_lvl_emerg, key, fmt, ##args)
#else
#define EMERG_LOG(fmt, args...)
#define KEMERG_LOG(key, fmt, args...) 
#endif
...
#define ALERT_LOG(fmt, args...) \
		DETAIL(ice::log_lvl_alert, 0, fmt, ##args)
/**
 * @def KALERT_LOG
...
#define KALERT_LOG(key, fmt, args...) \
		DETAIL(ice::log_lvl_alert, key, fmt, ##args)
#else
#define ALERT_LOG(fmt, args...)
#define KALERT_LOG(key, fmt, args...) 
#endif
...
#define CRIT_LOG(fmt, args...) \
		DETAIL(ice::log_lvl_crit, 0, fmt, ##args)
/**
 * @def KCRIT_LOG
...
#define KCRIT_LOG(key, fmt, args...) \
		DETAIL(ice::log_lvl_crit, key, fmt, ##args)
#else
#define CRIT_LOG(fmt, args...)
#define KCRIT_LOG(key, fmt, args...) 
#endif
...
#define ERROR_LOG(fmt, args...) \
		DETAIL(ice::log_lvl_error, 0, fmt, ##args)
/**
 * @def KERROR_LOG
...
#define KERROR_LOG(key, fmt, args...) \
		DETAIL(ice::log_lvl_error, key, fmt, ##args)
#else
#define ERROR_LOG(fmt, args...)
#define KERROR_LOG(key, fmt, args...) 
#endif
...
#define WARN_LOG(fmt, args...) \
		SIMPLY(ice::log_lvl_warning, 0, fmt, ##args)
/**
 * @def KWARN_LOG
...
#define KWARN_LOG(key, fmt, args...) \
		SIMPLY(ice::log_lvl_warning, key, fmt, ##args)
#else
#define WARN_LOG(fmt, args...)
#define KWARN_LOG(key, fmt, args...) 
#endif
...
#define NOTI_LOG(fmt, args...) \
		SIMPLY(ice::log_lvl_notice, 0, fmt, ##args)
/**
 * @def KNOTI_LOG
...
#define KNOTI_LOG(key, fmt, args...) \
		SIMPLY(ice::log_lvl_notice, key, fmt, ##args)
#else
#define NOTI_LOG(fmt, args...)
#define KNOTI_LOG(key, fmt, args...) 
#endif
...
#define INFO_LOG(fmt, args...) \
		SIMPLY(ice::log_lvl_info, 0, fmt, ##args)
/**
 * @def KINFO_LOG
...
#define KINFO_LOG(key, fmt, args...) \
		SIMPLY(ice::log_lvl_info, key, fmt, ##args)
#else
#define INFO_LOG(fmt, args...)
#define KINFO_LOG(key, fmt, args...) 
#endif
...
#define DEBUG_LOG(fmt, args...) \
		SIMPLY(ice::log_lvl_debug, 0, fmt, ##args)
/**
 * @def KDEBUG_LOG
...
#define KDEBUG_LOG(key, fmt, args...) \
		SIMPLY(ice::log_lvl_debug, key, fmt, ##args)
#else
#define DEBUG_LOG(fmt, args...)
#define KDEBUG_LOG(key, fmt, args...) 
#endif
...
#define TRACE_LOG(fmt, args...) \
		DETAIL(ice::log_lvl_trace, 0, fmt, ##args)
/**
 * @def KTRACE_LOG
...
#define KTRACE_LOG(key, fmt, args...) \
		DETAIL(ice::log_lvl_trace, key, fmt, ##args)
#else
#define TRACE_LOG(fmt, args...)
#define KTRACE_LOG(key, fmt, args...) 
#endif
